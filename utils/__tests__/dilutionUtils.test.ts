...
describe('dilutionUtils', () => {
  ...
  describe('getDilutionParts', () => {
    it('should extract parts correctly', () => {
      expect(getDilutionParts('1:9')).toEqual({ developerParts: 1, waterParts: 9 });
      expect(getDilutionParts('1+31')).toEqual({ developerParts: 1, waterParts: 31 });
      expect(getDilutionParts('2:8')).toEqual({ developerParts: 2, waterParts: 8 });
    });

    it('should handle stock solutions', () => {
      expect(getDilutionParts('Stock')).toEqual({ developerParts: 1, waterParts: 0 });
      expect(getDilutionParts('stock')).toEqual({ developerParts: 1, waterParts: 0 });
    });
  ...
  describe('describeDilution', () => {
    it('should create human-readable descriptions', () => {
      ...
      expect(describeDilution('1+31')).toBe('1 part developer + 31 parts water');
