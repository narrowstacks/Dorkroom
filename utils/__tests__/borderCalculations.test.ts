import { calculateBladeThickness, findCenteringOffsets, calculateOptimalMinBorder } from '../borderCalculations';
...
describe('borderCalculations utilities', () => {
  ...
  describe('findCenteringOffsets', () => {
    ...
    it('should find correct easel for standard paper (portrait - 8x10)', () => {
      if (!stdEasel) throw new Error("Standard easel (10x8) not found in constants");
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(stdWidth, stdHeight, false);
      ...
      expect(easelSize).toEqual({ width: stdEasel.width, height: stdEasel.height });
    });

    it('should find correct easel for standard paper (landscape - 10x8)', () => {
       if (!stdEasel) throw new Error("Standard easel (10x8) not found in constants");
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(stdWidth, stdHeight, true);
      expect(isNonStandardPaperSize).toBe(false); // 8x10 is considered standard via 10x8 easel
    ...
    const fittingEasel = EASEL_SIZES.find(e => e.width === 14 && e.height === 11)!; // Look for 14x11

    it('should find smallest fitting easel for non-standard paper (portrait - 9x12)', () => {
       if (!fittingEasel) throw new Error("Fitting easel (14x11) not found in constants");
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(nonStdWidth, nonStdHeight, false);
      expect(isNonStandardPaperSize).toBe(true);
      // 9x12 fits directly into 14x11 (no easel flip)
      expect(easelSize).toEqual({ width: fittingEasel.width, height: fittingEasel.height });
    });

    it('should find smallest fitting easel for non-standard paper (landscape - 12x9)', () => {
       if (!fittingEasel) throw new Error("Fitting easel (14x11) not found in constants");
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(nonStdWidth, nonStdHeight, true);
      expect(isNonStandardPaperSize).toBe(true);
    ...
    it('should return paper dimensions if no easel fits (portrait)', () => {
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(largeWidth, largeHeight, false);
      expect(isNonStandardPaperSize).toBe(true);
      expect(easelSize).toEqual({ width: largeWidth, height: largeHeight });
    });

     it('should return paper dimensions if no easel fits (landscape)', () => {
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(largeWidth, largeHeight, true);
      expect(isNonStandardPaperSize).toBe(true);
    ...
    const matchingEasel = EASEL_SIZES.find(e => e.width === 14 && e.height === 11)!;

     it('should handle paper exactly matching an easel size (portrait - 14x11)', () => {
       if (!matchingEasel) throw new Error("Matching easel (14x11) not found in constants");
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(matchEaselWidth, matchEaselHeight, false);
      expect(isNonStandardPaperSize).toBe(false); // 14x11 is a standard size
      // Paper 14x11, fits 14x11 easel
      expect(easelSize).toEqual({ width: matchingEasel.width, height: matchingEasel.height });
    });

     it('should handle paper exactly matching an easel size (landscape - 11x14)', () => {
       if (!matchingEasel) throw new Error("Matching easel (14x11) not found in constants");
      const { easelSize, isNonStandardPaperSize } = findCenteringOffsets(matchEaselWidth, matchEaselHeight, true);
      expect(isNonStandardPaperSize).toBe(false); // 14x11 is a standard size
  ...
  describe('calculateOptimalMinBorder', () => {
    ...
    const checkBordersForQuarterInch = (paperW: number, paperH: number, ratioW: number, ratioH: number, optimalBorder: number) => {
    ...
    it('should find a border close to 0.25 increments', () => {
      ...
      const optimalBorder = calculateOptimalMinBorder(paperW, paperH, ratioW, ratioH, currentMinBorder);
      expect(checkBordersForQuarterInch(paperW, paperH, ratioW, ratioH, optimalBorder)).toBe(true);
    });

    it('should find an optimal border when input is already near optimal', () => {
      const nearOptimalBorder = 0.75;
      const optimalBorder = calculateOptimalMinBorder(paperW, paperH, ratioW, ratioH, nearOptimalBorder);
    ...
    it('should return the current border if calculation leads to invalid sizes', () => {
      ...
      const optimalBorder = calculateOptimalMinBorder(paperW, paperH, ratioW, ratioH, largeMinBorder);
    ...
    it('should handle zero ratio height gracefully', () => {
        ...
        const optimalBorder = calculateOptimalMinBorder(paperW, paperH, ratioW, 0, currentMinBorder);
