 ...
 */
function convertToML(volume: number, unit: VolumeUnit, filmFormat: FilmFormat, numberOfRolls: number): number {
  switch (unit) {
    case 'ml':
      return volume;
    case 'oz':
    ...
    case 'rolls':
      const mlPerRoll = filmFormat === '35mm' ? ML_PER_35MM_ROLL : ML_PER_120_ROLL;
 ...
 */
function formatVolume(volumeML: number, displayUnit: VolumeUnit = 'ml'): string {
  switch (displayUnit) {
    case 'ml':
...
export const useChemistryCalculator = (): ChemistryCalculatorState & ChemistryCalculatorActions => {
  // Input state
  ...
  const { calculation, isValid, errors } = useMemo(() => {
    ...
    if (errors.length === 0 && selectedDilution) {
      ...
      const totalVolumeML = convertToML(inputVolume, unit, filmFormat, rollsNum);
