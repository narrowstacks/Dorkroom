    ...
    <VStack space="lg">
      ...
      <VStack space="sm" style={{
        ...
        <HStack style={{ alignItems: 'center', marginBottom: 12 }}>
          ...
          <Text style={{ fontSize: 16, fontWeight: '600', color: textColor, marginLeft: 8 }}>
            Recipe Ready
          </Text>
        </HStack>
        
        <Text style={{ fontSize: 14, color: textColor, opacity: 0.9, marginBottom: 8 }}>
          <Text style={{ fontWeight: '600' }}>Recipe:</Text> {formData.name || 'Untitled Recipe'}
        </Text>
        
        {formData.useExistingFilm ? (
          <Text style={{ fontSize: 14, color: textColor, opacity: 0.9, marginBottom: 8 }}>
            <Text style={{ fontWeight: '600' }}>Film:</Text> Selected from database
          </Text>
        ) : (
          <Text style={{ fontSize: 14, color: textColor, opacity: 0.9, marginBottom: 8 }}>
            <Text style={{ fontWeight: '600' }}>Film:</Text> {formData.customFilm?.brand} {formData.customFilm?.name} (Custom)
          </Text>
        )}
        
        {formData.useExistingDeveloper ? (
          <Text style={{ fontSize: 14, color: textColor, opacity: 0.9, marginBottom: 8 }}>
            <Text style={{ fontWeight: '600' }}>Developer:</Text> Selected from database
          </Text>
        ) : (
          <Text style={{ fontSize: 14, color: textColor, opacity: 0.9, marginBottom: 8 }}>
            <Text style={{ fontWeight: '600' }}>Developer:</Text> {formData.customDeveloper?.manufacturer} {formData.customDeveloper?.name} (Custom)
          </Text>
        )}
        
        <Text style={{ fontSize: 14, color: textColor, opacity: 0.9 }}>
          <Text style={{ fontWeight: '600' }}>Settings:</Text> {formData.temperatureF}°F, {formData.timeMinutes}min, ISO {formData.shootingIso}
          {formData.pushPull !== 0 && `, ${formData.pushPull > 0 ? '+' : ''}${formData.pushPull} stops`}
        </Text>
      </VStack>
      ...
      <VStack space="md">
        ...
        >
          ...
          <ButtonText style={{ marginLeft: 12, fontSize: 16, fontWeight: '600', color: "#fff" }}>
        ...
        {formData.isPublic && onSubmitToGitHub && (
          ...
          >
            ...
            <ButtonText style={{ marginLeft: 12, fontSize: 16, fontWeight: '600', color: developmentTint }}>
              Submit to GitHub
            </ButtonText>
          </Button>
        )}
        ...
        {recipe && onDelete && (
          <VStack space="sm" style={{ marginTop: 20 }}>
            ...
            >
              ...
              <ButtonText style={{ marginLeft: 10, fontSize: 14, color: '#ff4444' }}>
